...
def _run_speak_mode(model, tok, dev, bad_ids,
    ...
    Flow: moderator intro → Biden speaks → mic-listen for Trump → repeat.
...
def _run_listen_mode(model, tok, dev, bad_ids,
    ...
    Flow: hear moderator intro → hear opponent → Biden responds → repeat.
    No filesystem sync — communication is purely through audio.
    """
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Biden AI Debate Terminal",
    ...
    p.add_argument(
        ...
        help=(
            "speak  — moderator intro + Biden speaks first each exchange\n"
            "listen — waits for Trump, then responds  (default: speak)"
        ),
    )
    p.add_argument(
...
def run_debate(args) -> None:
    """Entry point for the Biden debate terminal."""
    model_dir = args.biden_model
